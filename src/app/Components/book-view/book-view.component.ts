...
export class BookViewComponent implements OnInit {
  ...
  requested: boolean =
    this.bookData.isActive && this.bookData.isActive != 0 ? true : false;
  ...
  updateBook(name: string, author: string, description: string, image: string) {
    ...
    if (image) this.bookData.coverImage = image;
  }
  updateDates() {
    if (this.user.role == false && this.currentIssue) {
      ...
      if (
        this.currentIssue.returnDate &&
        this.currentIssue.returnDate != 'null'
      ) {
  ...
  editBook() {
    const dialogRef = this.dialog.open(BookEditDialogComponent, {
      ...
      data: {
        ...
        id: this.bookData.id,
        issues: this.bookData.issues,
        callback: (
          name: string,
          author: string,
          description: string,
          image: string
        ) => this.updateBook(name, author, description, image),
      },
    });
  }
  toggleRequested() {
  ...
  returnBook() {
    ...
    if (days < 0) fine = 0;
    else fine = days * multiplier;
    const dialogRef = this.dialog.open(PostIssueComponent, {
      width: '550px',
      data: {
        type: 1,
        onSubmit: () => {
          this.service.putIssue(
            this.currentIssue.issueId,
            this.user.id,
            this.bookData.id,
            0,
            this.bookData.issueDate,
            this.bookData.expiryDate,
